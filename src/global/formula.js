...
import { isInlineStringCell,  convertSpanToShareList,convertSpanToShareString } from "../controllers/inlineString";
...
const luckysheetformula = {
    ...
    updatecell: function (r, c, value, isRefresh = true) {
        ...
        if (!value && !isCurInline && isPrevInline) {
            ...
            curv.ct.t = "inlineStr";
            curv.ct.s = convertSpanToShareList($input.find("span"));
            debugger;
            if (isCopyVal) {
                curv.ct.s = [
                    {
        ...
        if (getObjType(curv) == "object") {
            ...
            } else { // 如果有span 标签的话  // hz_flag
                //  核心思想就是 获取dom 对应的sapn 下的内容，如果有b 是加粗，i 是斜体，u 是下划线
                // 如果有这些就将curv 下的 s 数组中对应下标（和span 的下标位置一样）的属性加上
                // b 加属性 bl:1,
                // i 加属性 it:1,
                // u 加属性 un:1
                // 获取包含内容的span元素
                //  最终的值是设置curv.ct.s 对应下标的属性中
             // 获取包含内容的span元素
             const spans = document.querySelectorAll('#luckysheet-rich-text-editor span');

             // 确保curv.ct存在
             curv.ct = curv.ct || {};
             curv.ct.s = curv.ct.s || []; // 确保curv.ct.s存在，若不存在则初始化为空数组
             
             // 遍历每个span元素，检查其样式
             let my_index = 0;
             spans.forEach((span, index) => {
                debugger;
                const text_content = span.textContent;
                 // 获取span的HTML内容并分割为子元素（包括文本和标签）
                 const children = [...span.childNodes]; // 获取所有子节点
                 // 初始化当前行的样式
                 let currentStyle = {
                     bl: span.tagName === 'B' ? 1:0, // 加粗
                     it: span.tagName === 'I' ?1:0, // 斜体
                     un: span.tagName === 'I' ?1:0, // 下划线
                     v: ''  // 文本内容
                 };
                 ...
                 children.forEach(child => {
                    ...
                    let add_flag = false;
                     ...
                     if (child.nodeType === Node.TEXT_NODE) {
                        ...
                        if (pre_child_type  ==1){ // 上一次是文本 这一次是标签，说明换一行了，直接
                            my_index++;
                            add_flag = true;
                        }
                        now_child_type = 0;
                         // 对于包含标签的情况，处理对应的文本和样式
                         const text = child.textContent.trim();
                         const isBold = child.tagName === 'B';
                         const isItalic = child.tagName === 'I';
                         const isUnderline = child.tagName === 'U';
             
                         if (text) {
                             currentStyle.v += text; // 拼接文本内容
                             // 只在对应样式属性不存在时进行赋值
                             if (isBold) currentStyle.bl = 1; // 加粗
                             if (isItalic) currentStyle.it = 1; // 斜体
                             if (isUnderline) currentStyle.un = 1; // 下划线
                         }
                     }
                     if (pre_child_type != null && pre_child_type != now_child_type ){
                        const gap = now_child_type - pre_child_type;
                        if (gap ===1 && !add_flag){
                            different_content = true;
                            my_index++;
                        }
                     }
                     pre_child_type = now_child_type
                 });
                 debugger;
                 // 将当前样式应用到 curv.ct.s[index]，只修改需要的属性
                ...
                //  curv.ct.s[index].v = (curv.ct.s[index].v || '') + currentStyle.v; // 文本内容拼接
             });
